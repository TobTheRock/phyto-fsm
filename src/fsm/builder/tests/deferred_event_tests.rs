...
fn inherited_deferred_events() {
    ...
    let parent = builder.add_transition(TransitionParameters::Enter {
        target: parent_name,
    });
...
fn child_transition_overrides_parent_deferred_event() {
    let mut builder = UmlFsmBuilder::new("test");
    let parent = builder.add_transition(TransitionParameters::Enter { target: "Parent" });
...
fn multi_level_inheritance() {
    let mut builder = UmlFsmBuilder::new("test");
    let grandparent = builder.add_transition(TransitionParameters::Enter {
        target: "Grandparent",
    });
